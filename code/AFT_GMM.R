...
SimDataAFT <- function(n.sample = 100, T0 = 0.5,
                       aux = FALSE, option = 1) {
  # TODO:
  Xcut <- function(w, v){ 
  ...
  ZB = beta[1] + z1 * beta[2] + z2 * beta[3]
  t = rweibull(n.sample, shape = 1/sigma, scale = exp(ZB))
  Acut0 <- Xcut(z1, z2 )
  grpid = Acut0  %*% c(1, 2)
...
# x = rnorm(1000, 1, 2)
# y = rweibull(1000, shape = 2, scale = 5) * exp(x)
# dat = data.frame(x = x, y = y)  
# survreg(Surv(y) ~ x, dat, dist ="weibull")